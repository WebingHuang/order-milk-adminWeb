      ...
      <a-card :bordered="false">
        ...
        >
          ...
          <span slot="pic" slot-scope="text, record" type="primary" @click="enlargeImg(record)">
            <img style="width: 150px; " :src="record.imgUrl" />
          </span>
          ...
          >
            ...
            >
              ...
              <a-form-model-item label="上传图片" >
                ...
                >
                  <img v-if="preview" :src="swiperForm.imgUrl"  />
              ...
              <a-form-model-item label="商品名称" >
                <!-- <a-input v-model="swiperForm.spuName" /> -->
                <a-select mode="combobox" style="width: 100%"    v-model="swiperForm.spuName" @blur="holderCodeBlur" @select="holderCodeBlur">
      ...
      <div>
        ...
        <a-modal v-model="enlarge" title="预览" @ok="en" okText="完成" width="500px" cancelText="关闭">
          <div style="width:100%;display: flex;justify-content: center;">
            <img style="width: 200px; " :src="icooon" class="preview" />
          </div>
        </a-modal>
      </div>
    </page-header-wrapper>
...
import {  getSwperListPage ,editSwiper ,updateStatus ,delSwiper,getCommodityList} from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    holderCodeBlur(){
      var _this = this;
      if(_this.swiperForm.spuName){
        for(var i = 0; i <_this.commodityData.spuName.length; i++){
          if(_this.swiperForm.spuName == _this.commodityData.spuName[i]){
            _this.swiperForm.spuId =  _this.commodityData.spuId[i];
            _this.swiperForm.spuImg =  _this.commodityData.spuImg[i];

            break;
          }
        }
      }else{
          _this.swiperForm.spuId = '';
      }
    ...
    handleAdd() {
      ...
      this.swiperForm.spuId ='';
      this.swiperForm.spuName ='';
    ...
    handleEdit(record) {
      ...
      this.swiperForm.spuId = record.spuId;
      this.swiperForm.spuName = record.spuName;
      ...
      this.$refs.swiperForm.validate((valid) => {
        ...
        if (!valid) {
          ...
          return editSwiper(this.state,this.state?
          {
            ...
            spuId:this.swiperForm.spuId, //
            spuName:this.swiperForm.spuName, //
            spuImg: this.swiperForm.spuImg, //
