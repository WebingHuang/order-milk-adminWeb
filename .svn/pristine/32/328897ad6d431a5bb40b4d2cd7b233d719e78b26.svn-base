  ...
  <div id="sysnotice">
    <page-header-wrapper>
      ...
      <a-card :bordered="false">
        ...
        <!-- 新建 -->
        <div class="table-operator" :title="state ? '修改通知' : '新增通知'">
        ...
        <div>
          ...
          >
            <a-form-model
              ...
              ref="ruleForm"
            >
              <a-form-model-item label="剩余支数" :required="true">
                <a-input v-model="ruleForm.residueNum" placeholder="小于等于该数则通知"/>
              </a-form-model-item>
              <a-form-model-item label="通知内容">
                <a-input v-model="ruleForm.note" />
              </a-form-model-item>
...
import { TreeSelect } from 'ant-design-vue' // 选中所有子节点,只渲染父节点名称,如果取消,点哪个渲染哪个

import { getQueryInform, getPower, addInform, deteleInform } from '@/api/system'
  ...
  data () {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return getQueryInform(requestParameters, this.queryCondition)
          .then(res => {
            if (res.code == 200) {
              return {
                data: res.data.map(e => Object.assign(e, {
                  residueNumPrex: e.residueNum + '支'
                })),
  ...
  methods: {
    ...
    deleteAll (record) {
      ...
      if (this.selectedRowKeys.length > 0) {
        ...
        this.$confirm({
          ...
          onOk() {
            return Promise.all(
              that.selectedRowKeys.map(e => deteleInform({
                id: e
              }))
            ).then(() => {
    ...
    handleOk (e) {
      this.$refs.ruleForm.validate(valid => {
        ...
        if (this.state) {
          data['residueNum'] = this.ruleForm.residueNum
          data['note'] = this.ruleForm.note
        } else {
          data['residueNum'] = this.ruleForm.residueNum
          data['note'] = this.ruleForm.note
        }
        ...
        if (!valid) {
          ...
          return addInform(data).then(res => {
            this.$refs.table.refresh(false)
            this.visible = false
            this.$notification.success({
              message: '提示',
