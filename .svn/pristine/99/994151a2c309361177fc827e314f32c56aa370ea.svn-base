            ...
            <a-form-model :model="info" :label-col="labelCol" :wrapper-col="wrapperCol">
              ...
              <a-form-model-item label="营业执照">
                <img
                  ...
                  @click="lookimg(info.businessLicense)"
                />
              </a-form-model-item>
...
// 引入调用接口方法
import { getEnterpriseList, getInfoById, infoAdopt } from '@/api/seniority'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return getEnterpriseList(requestParameters, this.queryCondition)
          .then((res) => {
            if (res.state == 2000) {
              let pageData = {
  ...
  methods: {
    ...
    //审核状态
    handleChangea(value) {
      this.queryCondition.companyAdopt = value.key
