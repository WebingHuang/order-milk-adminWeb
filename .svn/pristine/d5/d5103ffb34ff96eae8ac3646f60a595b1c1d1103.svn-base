...
// 引入调用接口方法
import { getCompanyListByUserId, editUser, deleteCompany, filesCompany, typeCompany } from '@/api/company'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return getCompanyListByUserId(requestParameters, this.queryCondition)
          .then((res) => {
            if (res.state == 2000) {
              const pageData = {
  ...
  methods: {
    ...
    handleDel(record) {
      ...
      return deleteCompany({ idArr: record.id, oldImg: imgLIst.join(','),userId:this.companyForm.userId}).then((res) => {
    ...
    deleteAll(record) {
      ...
      if (this.selectedRowKeys.length > 0) {
        ...
        this.$confirm({
          ...
          content: '确定要全部删除吗?',
          onOk() {
            return deleteCompany({ idArr: that.selectedRowKeys.join(','), oldImg: imgLIst.join(','),userId:that.companyForm.userId}).then((res) => {
