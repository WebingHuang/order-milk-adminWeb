      ...
      <a-card :bordered="false">
        ...
        >
          ...
          <span slot="enable" slot-scope="text, record" type="primary" >
             ...
             <a-switch checked-children="开" un-checked-children="关" :checked='record.publishStatus==1' @click="unloading(record)" />

          </span>


          <span slot="enState" slot-scope="text, record" type="primary" >

             <a-switch checked-children="开" un-checked-children="关" :checked='record.homeStatus==1' @click="unloadings(record)" />
        ...
        <div>
          ...
          >
            <a-form-model
              ...
              ref="swiperForm"
            >
              <a-form-model-item label="图片标题" prop="imgDesc">
...
import { editSwiper ,updateStatus } from '@/api/system'



import {getCommodityListPage ,unloading ,deleteCommodity ,editIsIndex} from '@/api/commodity'
...
const columns = [
  ...
  //   width: '50px',
  //   dataIndex: 'isCard',
  //   scopedSlots: { customRender: 'isCards' },
  //   align: 'center',
  // },

  {
  ...
  },
   {
    ...
    scopedSlots: { customRender: 'enState' },
    align: 'center',
  },
{
    title: '发布时间',
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return getCommodityListPage(requestParameters, this.queryCondition)
          .then((res) => {
            if (res.code == 200) {
  ...
  methods: {
    ...
    handleEdit(record) {
      ...
      this.$router.push('/commodity/edit?id='+record.id);
