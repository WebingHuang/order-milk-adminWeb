    ...
    <page-header-wrapper>
      ...
      <a-card :bordered="false">
        ...
        >
          ...
          <div slot="action" slot-scope="text, record" style="word-break: keep-all;margin:auto;">
            <template>
              ...
              <div style="text-align: left;">
                ...
                <span style="white-space: nowrap;">
                  <a @click="showDeliverCard(record, 2)" v-if="record.orderStatus==1||record.orderStatus==2">送货</a>
                  <a-divider type="vertical" v-if="record.orderStatus==1||record.orderStatus==2" />
                  <a @click="showDeliverCard(record, 1)" 
                    v-if="record.orderStatus==3||record.orderStatus==2||record.orderStatus==1||record.orderStatus==4||record.orderStatus==5" 
                  >送货记录</a>
                </span>
      ...
      <template>
        ...
        >
          <deliver-goods 
          @confirm="resetList()"
          @cancel="showDeliver=false" :orderId="orderId" :dataObj="dataObj"></deliver-goods>
        </a-drawer>
      </template>
...
import {getFindByOrderListPage,updateOrderStatus, updateOrderSettlement ,querySchoolClassNameByOrder, getOrderInfoById ,getOrderExcel, exportPieSalaryRecordExcel,putOderState} from '@/api/system'
...
export default {
  ...
  methods: {
    ...
    getSchoolList() {
      this.orderExl.schoolName = ''
      return querySchoolClassNameByOrder({
        pageNum: 1,
        pageSize: 50,
        orderStatus: this.orderExl.orderStatus,
        beginDate:this.orderExl.beginDate + ' 00:00:00',
        endDate: this.orderExl.endDate + ' 23:59:59',
      }, {
      })
      .then((res) => {
        if(res.code==200){
    ...
    showDeliverCard(record, type) {
      this.orderId = record.id;
      this.showDeliver = true; 
      this.dataObj = null;
      if (type==1) {
        this.deliverTitle = '送货记录';
      } else {
        this.dataObj = record;
        this.deliverTitle = '去送货';
      }
    },
    //行
    getItemInfo(item){
    ...
    excelChangea(value){
      ...
      if (this.orderExl.orderStatus != value) {
        this.getSchoolList();
      }
    ...
    handleEdit(record) {
      ...
      this.orderFrom.id = record.id;
      this.orderFrom.orderStatus = record.orderStatus;
      // this.orderFrom.deliverySn = record.deliverySn;
      this.orderFrom.settlement = record.isSettlement;
      console.log(record.orderStatus)
      this.upddataStateList = this.statusList;
      // if (record.orderStatus == 0) {
      //   this.upddataStateList.push(this.statusList[0])
      // }

      // if(record.orderStatus == 1 || record.orderStatus == 2 || record.orderStatus == 3){ //待发货状态
    ...
    oneKeyExportExcl(){
      ...
      if (!_this.orderExl.beginDate) {
        ...
        .then((res) => {
         ...
         // _this.exclName = "订单-"+datatime+".xls";l
        ...
        if (!valid) {
          ...
          return Promise.all([
            updateOrderSettlement({
              id: this.orderFrom.id,
              settlement: this.orderFrom.settlement,
            }),
            updateOrderStatus(this.orderFrom)
          ]).then((res) => {
            if(res[1].code == 200){
              this.$refs.table.refresh(false)
              this.visible = false
