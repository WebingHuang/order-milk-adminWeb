        ...
        <div>
          ...
          >
            <a-form-model
              ...
              ref="orderFrom"
            >
              <a-form-model-item label="每日提现总额度"  :required="true">
                <a-input v-model="orderFrom.combination" type="number"/>
              </a-form-model-item>

              <a-form-model-item label="最大提现金额" :required="true">
                <a-input v-model="orderFrom.max" type="number" />
              </a-form-model-item>

               <a-form-model-item label="最小提现金额" :required="true">
                <a-input v-model="orderFrom.min" type="number"/>
              </a-form-model-item>
               <a-form-model-item label="每日提现次数" :required="true">
                <a-input v-model="orderFrom.number" type="number"/>
              </a-form-model-item>
              <a-form-model-item label="提现费率" :required="true">
                <a-input v-model="orderFrom.rate" type="number" />
              </a-form-model-item>
...
import { getOrderInfoById ,getRateListPage,editRate,editRateStatus ,removeEateById} from '@/api/system'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return getRateListPage(requestParameters, this.queryCondition)
          .then((res) => {
            console.log(res)
  ...
  methods: {
    ...
    handleEdit(record) {
      ...
      this.orderFrom.max = record.max;
      this.orderFrom.min = record.min;
      this.orderFrom.number = record.number;
      this.orderFrom.rate = record.rate;
      this.orderFrom.combination = record.combination;
    ...
    handleDel(record) {
      ...
      if(record.status == 1){
        this.$notification.error({
          message: '提示',
          description: '启用状态不能删除',
        })
        return
      }

      return removeEateById({ id: record.id }).then((res) => {
        this.$refs.table.refresh(false)
        this.$notification.success({
          message: '提示',
    ...
    deleteAll(record) {
      ...
      if (this.selectedRowKeys.length > 0) {
        ...
        this.$confirm({
          ...
          content: '确定要全部删除吗?',
          onOk() {
            return removeEateById({ id: that.selectedRowKeys.join(',') }).then((res) => {
    ...
    handleOk(e) {
      ...
      this.$refs.orderFrom.validate((valid) => {
        ...
        if (!valid) {
          ...
          return editRate(this.state,this.orderFrom).then((res) => {
