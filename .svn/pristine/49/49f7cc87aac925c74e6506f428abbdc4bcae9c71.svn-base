      ...
      <template>
        <div>
          <a-modal v-model="visiblea" title="审核信息" cancelText="未通过审核" okText="通过审核" width="800px">
            <a-form-model :model="shopvoucherInfo" :label-col="labelCol" :wrapper-col="wrapperCol">
              <a-form-model-item label="其它平台入驻名称">{{shopvoucherInfo.name}}</a-form-model-item>
              <a-form-model-item label="联系人">{{shopvoucherInfo.接触s}}</a-form-model-item>

              <a-form-model-item label="电话">{{shopvoucherInfo.phone}}</a-form-model-item>
              <a-form-model-item label="邮箱">{{shopvoucherInfo.mail}}</a-form-model-item>
              <a-form-model-item label="其他官方平台后台截图凭证">
                <img
                  v-for="(item,index) in shopvoucherInfo.voucherList"
                  :key="index"
                  :src="item"
              ...
              <a-form-model-item label="官方机构或平台授予奖项或荣誉的凭证">
                <img
                  v-for="(item,index) in shopvoucherInfo.honorList"
                  :key="index"
                  :src="item"
                  style="width:100px; height: 100px; margin-right: 10px;"
                  @click="lookimg(item)"
...
// 引入调用接口方法
import { getShopvoucherList, getShopvoucherById, shopvoucherAdopt } from '@/api/seniority'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return getShopvoucherList(requestParameters, this.queryCondition)
          .then((res) => {
            if (res.state == 2000) {
              let pageData = {
  ...
  methods: {
    ...
    // 审核按钮弹窗状态
    showModal(record) {
      return getShopvoucherById({ id: record.id }).then((res) => {
        if (res.state == 2000) {
          this.visiblea = true
          this.shopvoucherInfo = res.data
          if (res.data.message) {
            this.message = res.data.message
          }
          if (this.shopvoucherInfo.honorList) {
            this.shopvoucherInfo.honorList = this.shopvoucherInfo.honorList.split(',')

            console.log(this.shopvoucherInfo.honorList)
          }
          if (this.shopvoucherInfo.voucherList) {
            this.shopvoucherInfo.voucherList = this.shopvoucherInfo.voucherList.split(',')
          }
        }
      })
    },
    ...
    Save(val) {
      ...
      return shopvoucherAdopt({
        id: this.shopvoucherInfo.id,
        adopt: val,
