        ...
        <div>
          <a-modal v-model="visiblea" title="审核信息" cancelText="未通过审核" okText="通过审核" width="800px">
            <a-form-model :model="info" :label-col="labelCol" :wrapper-col="wrapperCol">
              <a-form-model-item label="用户ID">{{info.userId}}</a-form-model-item>
              <a-form-model-item label="人脸头像">
                <img
                  :src="info.face"
                  style="width:100px; height: 100px; margin-right: 10px;"
                  @click="lookimg(info.face)"
                />
              </a-form-model-item>
              <a-form-model-item label="身份证正面">
                <img
                  :src="info.identityJust"
                  style="width:100px; height: 100px; margin-right: 10px;"
                  @click="lookimg(info.identityJust)"
                />
              </a-form-model-item>
              <a-form-model-item label="身份证反面">
                <img
                  :src="info.identityBack"
                  style="width:100px; height: 100px; margin-right: 10px;"
                  @click="lookimg(info.identityBack)"
                />
              </a-form-model-item>
...
// 引入调用接口方法
import { getPersonalList, getInfoById, infoAdopt } from '@/api/seniority'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return getPersonalList(requestParameters, this.queryCondition)
          .then((res) => {
            if (res.state == 2000) {
              let pageData = {
  ...
  methods: {
    ...
    //审核状态
    handleChangea(value) {
      this.queryCondition.personalAdopt = value.key
      this.$refs.table.refresh(false)
    },
    // 审核按钮弹窗状态
    showModal(record) {
      return getInfoById({ id: record.id }).then((res) => {
        if (res.state == 2000) {
          this.visiblea = true
          this.info = res.data
          if (res.data.message) {
