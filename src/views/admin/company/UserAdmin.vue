...
// 增删改查接口
import { getUserListByCompanyId, editUser, deleteUser, getUploadImgUrl, getRole } from '@/api/system'
// 引入图片裁剪组件
...
export default {
   ...
   data() {
      ...
      return {
         ...
         // 查询参数
         queryCondition: {
            companyId: this.$store.getters.userInfo.userInfo.companyId,
         },
         // 分页参数
         pageFrom: {
            // pageNum: 1, //第几次请求
         ...
         loadData: (parameter) => {
            ...
            return getUserListByCompanyId(requestParameters, this.queryCondition)
               .then((res) => {
                  if (res.state == 2000) {
                     const pageData = {
   ...
   methods: {
      // 获取用户的id
      //    getLocalStorage() {
      //    this.queryCondition.companyId = JSON.parse(localStorage.getItem('cf01'))
      //    console.log(this.queryCondition.companyId);
      //  },
      // 角色列表下拉框选择事件
      ...
      handleChange({ file, fileList }) {
         ...
         if (file.status == 'uploading') {
            ...
            // 上传图片接口
            return getUploadImgUrl(myform).then((res) => {
               // 返回图片链接
      ...
      handleDel(record) {
         console.log(record)
         return deleteUser({ idArr: record.id, imgList: record.photoSrc, type: 5 }).then((res) => {
            // console.log(res)
      ...
      async handleOk(e) {
         this.$refs.ruleForm.validate(async (valid) => {
            ...
            this.ruleForm['companyId'] = this.$store.getters.userInfo.userInfo.companyId
