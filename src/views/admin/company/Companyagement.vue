              ...
              <a-col :md="(!advanced && 8) || 24" :sm="24">
                ...
                >
                  <a-select-option value="">
                    <a-badge status="success" text="全部公司" color="#2db7f5" />
                  </a-select-option>
                  <a-select-option value="1">
                    <a-badge status="default" text="待认证公司" />
                  </a-select-option>
                  <a-select-option value="2">
                    <a-badge status="success" text="已认证公司" />
                  </a-select-option>
            ...
            <a-modal v-model="visiblea" title="审核信息"  cancelText="未通过审核"  okText="通过审核">
                  <a-form-model :model="companyForm" :label-col="labelCol" :wrapper-col="wrapperCol">
                    ...
                    <a-form-model-item label="Logo" v-if="state" >
                      <img :src="companyForm.icon" style="width:100px; height: 100px;" @click="lookimg(companyForm.icon)">
                    </a-form-model-item>
                    <a-form-model-item label="添加人" v-if="!state">
                      {{companyForm.userId}}
                    </a-form-model-item>
                    <a-form-model-item label="公司介绍" v-if="state">
                      {{companyForm.introduce}}
                    </a-form-model-item>
                    <a-form-model-item label="id" v-if="idfalse">
                      {{forcompanyForm.id}}
                    </a-form-model-item>
                    <a-form-model-item label="营业执照">
                      <img :src="companyForm.licenseUrl" style="width:200px; height: 200px;" @click="lookimg(companyForm.licenseUrl)">
                    </a-form-model-item>
                    <a-form-model-item label="描述">
                      {{companyForm.describe}}
                    </a-form-model-item>
                  </a-form-model>
...
data() {
  ...
  methods: {
    ...
    //审核状态
      handleChangea (value) {
      this.queryCondition.authentication = value.key;
    ...
    // 审核
    Save(val) {
      return stateCompany({id:this.companyForm.id,authentication:val}).then((res) => {
              this.visiblea = false;
              this.$refs.table.refresh(false)
