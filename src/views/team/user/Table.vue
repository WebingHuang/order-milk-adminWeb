    ...
    <page-header-wrapper>
      ...
      <a-card :bordered="false">
            ...
            <a-row :gutter="48">
              ...
              <a-col :md="6" :sm="24">
                <a-form-item label="邀请码">
                  <a-input v-model="queryCondition.saltTwo" placeholder="" />
                </a-form-item>
              </a-col> -->
              <!-- <a-col :md="6" :sm="24">
        ...
        <!-- 表格 -->
          ...
          <a-table :columns="columns" :data-source="loadData" rowKey="id">
          ...
          >
            ...
            >
              <a-form-model-item label="手机号" prop="phone">
                <a-input v-model="userForm.phone"  :disabled="true"/>
              </a-form-model-item>
              ...
              <a-form-model-item label="用户名称" prop="userName">
                <a-input v-model="userForm.userName" :disabled="true" />
              </a-form-model-item>
              <a-form-model-item label="角色类型"  :required="true">


                <a-select mode="tags" style="width: 100%" placeholder="(支持多角色)" @change="typeTreea" v-model="userForm.roleCode" :disabled="true">
              ...
              <a-form-model-item label="社区代理商" >
                <a-switch :default-checked="isAgent" @change="onAgentChange" />
              </a-form-model-item>
...
import {getAgentUser,editUser,getRoleTypeList,removeUser ,editUserStatus,getCardListByUserCOde} from '@/api/system'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      //   return getAgentUser(requestParameters, this.queryCondition)
      //     .then((res) => {

      //       console.log(res.data.overallUser.sysUserEntities)
      //       if (res.code == 200) {

      //         const pageData = {
      //           data: res.data.overallUser.sysUserEntities,
      //           pageNo: 1 ,
      //           pageSize: 10,
      //           totalCount: Number(res.data.overallUser.sysUserEntities.length),
  ...
  methods: {
    ...
    getAgentUser(){
        ...
        return getAgentUser(this.queryCondition)
          .then((res) => {

            console.log()
            if (res.code == 200) {

              this.loadData = res.data.overallUser.sysUserEntities;
    ...
    onAgentChange(checked){
      if(checked){

          let isR = false;
          this.userForm.roleCode.map(item=>{
            if(item == "d2fe397f-2d95-4532-a1eb-74efbe242659"){
              isR = true;
            }
          })
          if(!isR){
            //不包含
            this.userForm.roleCode.push('d2fe397f-2d95-4532-a1eb-74efbe242659');
          }
      }else{
          this.userForm.roleCode.map((item,index)=>{
            if(item == "d2fe397f-2d95-4532-a1eb-74efbe242659"){
               this.userForm.roleCode.splice(index,1);
            }
          })
      }
